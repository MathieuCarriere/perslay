...
def perslay(output, name, diag,
    ...
    """
        ...
        diag : big matrix of shape [N_diag, N_pts_per_diag, dimension_diag (coordinates of points) + 1 (mask--either 0 or 1)]
    ...
    if persistence_weight == "linear":
        with tf.variable_scope(name + "-linear_pweight"):
            C = tf.get_variable("C", shape=[1], initializer=coeff_init) if not coeff_const else tf.get_variable("C", initializer=coeff_init)
            weight = C * tf.abs(tensor_diag[:, :, 1:2])

    if persistence_weight == "grid":
        with tf.variable_scope(name + "-grid_pweight"):

            W = tf.get_variable("W", shape=grid_size, initializer=grid_init) if not grid_const else tf.get_variable("W", initializer=grid_init)
            indices = []
    ...
    if output_dim == 1:
        ...
        # Second layer of channel: fully-connected
        for idx, tup in enumerate(fc_layers):
            # tup is a tuple whose element are 1. dim of fully-connected, 2. string for processing, 3. (optional) dropout value
        ...
        for idx, tup in enumerate(cv_layers):
            # tup is a tuple whose element are 1. num of filters, 2. kernel size, 3. string for postprocessing, 4. (optional) dropout value
